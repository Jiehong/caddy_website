...
- **include** specifies the log names to be included in this logger. For example, to include only logs emitted by the admin API, you would include `admin.api`. By default, all logs are included (aka this list is empty by default).
- **exclude** specifies the log names to be excluded from this logger. For example, to exclude only HTTP access logs, you would exclude `http.log.access`. By default, no logs are excluded (aka this list is empty by default).

The logger names that `include` and `exclude` accept depend on the modules used, and the easiest way to find them out is from the logs.

Here is an example logging as json all http access logs and admin logs to stdout, but not http error logs:
